...
| 암호화 | 스토리지 암호화를 설정 할 수 있습니다. 스토리지 생성 후 암호화 설정 변경은 불가능합니다. 암호화 키 저장소 설정이 선행되어야 합니다. |
...
암호화 스토리지 생성 시 설정한 키 저장소에 대칭키가 추가됩니다. 키 저장소 ID 변경 시 기존 대칭키는 유지되며 변경 이후 생성되는 암호화 스토리지 대칭키가 변경된 키 저장소에 추가됩니다. NAS 서비스에 의해 추가된 대칭키는 암호화 스토리지 사용 중에 삭제할 수 없으며 암호화 스토리지 삭제 시 대칭키가 자동 삭제됩니다.

> [참고]
> 암호화 키 저장소를 설정하기 위해서는 사전에 Secure Key Manager 서비스를 활성화하고 키 저장소를 생성해야 합니다. [키 저장소 생성 안내](https://docs.nhncloud.com/ko/Security/Secure%20Key%20Manager/ko/getting-started/#_1)
>
> 키 저장소 ID는 키 저장소 상세 정보에서 확인할 수 있습니다. [키 저장소 상세 정보 안내](https://docs.nhncloud.com/ko/Security/Secure%20Key%20Manager/ko/getting-started/#_2)
>
> 암호화 설정 시 발생하는 키 사용량은 Secure Key Manager 서비스 요금 정책에 따라 비용이 청구됩니다. [Secure Key Manager 요금 안내](https://www.nhncloud.com/kr/service/security/secure-key-manager#price)
