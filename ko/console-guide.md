...
#### CIFS 인증 정보 관리
CIFS 프로토콜을 사용하기 위해서는 CIFS 인증 정보를 생성해야 합니다. 인증 정보는 프로젝트 단위로 관리되며, CIFS 스토리지마다 접근할 CIFS 인증정보를 등록해야 합니다.
...
스토리지 복제를 중지합니다.
복제를 중지하면 대상 스토리지가 쓰기 가능한 상태로 변경되며, 마운트해 사용할 수 있습니다.
...
파일 탐색기에서 좌측 탐색창에서 **내 PC**를 우클릭 후 **네트워크 드라이브 연결**을 선택합니다. 네트워크 드라이브 연결 창이 뜨면, 마운트 할 드라이브를 선택 후 폴더 경로를 입력합니다.  폴더 경로 형식은 `\\{스토리지 ip}\{스토리지 명}`와 같습니다. ex) \\\\192.168.0.100\\cifs