...
이 문서는 NHN Cloud 콘솔에서 NAS의 볼륨과 스냅숏을 관리하고 인스턴스에 연결하는 방법을 설명합니다.
...
새로운 볼륨을 생성합니다. 생성된 볼륨은 NFS(network file system, 네트워크 파일 시스템) 및 CIFS(common internet file system, 공용 인터넷 파일 시스템) 프로토콜을 사용하여 인스턴스에서 접근할 수 있습니다.
...
| 스냅숏 자동 생성 | 매일 1회 지정한 시간에 자동으로 생성합니다. 최대 저장 개수에 도달하면 자동으로 생성된 스냅숏 중 가장 먼저 생성된 스냅숏이 삭제됩니다.                                             |
...
!!! tip "알아두기"
    Secure Key Manager 서비스 요금 정책에 따라 키 저장소 사용 요금이 청구됩니다. 사용 요금에 관한 자세한 사항은 [Secure Key Manager 요금 안내](https://www.nhncloud.com/kr/service/security/secure-key-manager#price)를 참고합니다.
...
| 항목 | 설명 |
| --- | --- |
| 연결 정보 | 인스턴스에서 마운트 시 사용할 연결 경로를 표시합니다. |
...
복제를 설정하면 대상 위치에 원본 볼륨과 같은 크기의 대상 볼륨이 생성됩니다. 대상 볼륨은 읽기 전용 상태로 생성되며, 대상 볼륨의 상태를 변경하려면 복제를 중지하거나 복제 설정을 해제해야 합니다.
...
!!! tip "알아두기"
    볼륨 복제는 스냅숏을 사용하여 진행합니다. 복제 설정 시 `{볼륨명}.mirror.%` 형식의 스냅숏이 자동으로 생성되며 삭제할 수 없습니다.
...
!!! danger "주의"
    ...
    복제가 실행되면 대상 볼륨의 기존 데이터는 모두 삭제되고, 대상 볼륨은 원본 볼륨과 동일한 상태가 됩니다.
...
파일 탐색기의 왼쪽 탐색창에서 **내 PC**를 마우스 오른쪽 버튼으로 클릭한 후 **네트워크 드라이브 연결**을 선택합니다. 네트워크 드라이브 연결 창이 나타나면, 마운트할 드라이브를 선택 후 폴더 경로를 입력합니다.
폴더 경로 형식은 `\\{볼륨 ip}\{볼륨명}`과 같습니다.
