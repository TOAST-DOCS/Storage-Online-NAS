...
새로운 스토리지를 생성합니다. 생성된 스토리지는 NFS(네트워크 파일 시스템) 및 CIFS(공용 인터넷 파일 시스템) 프로토콜을 이용하여 인스턴스에서 접근할 수 있습니다.

| 항목 | 설명 |
| -- | -- |
| 이름 | 생성될 스토리지의 이름입니다. 스토리지 이름을 통해 NFS 또는 CIFS의 접근 경로를 만듭니다. 이름은 100자 이내의 영문자와 숫자, 일부 기호('-', '_')만 입력할 수 있습니다. |
| 설명 | 스토리지에 대한 설명입니다. |
...
| 스냅숏 예약 용량 | 스냅숏이 저장될 공간을 미리 할당합니다. 설정한 용량을 제외한 용량에 데이터 저장이 가능합니다. 0을 입력할 경우 스냅숏이 정상적으로 생성되지 않습니다. |
...
| 스냅숏 생성 시 스토리지 사용량 | 스냅숏 생성 시점의 NAS 스토리지 사용량입니다. 스냅숏을 복원할 경우 해당 크기로 복원됩니다. |
| 생성일 | 스냅숏이 생성된 시간입니다. |
...
> [주의]
> 서브넷 연결 추가 후 ACL에 서브넷 대역이 존재하지 않으면 마운트가 불가능 합니다.
...
| 복제 설정 상태 | 원본과 대상 스토리지 간 복제 설정 상태를 표시합니다.<br>Active: 활성화 상태<br>Retrieve failed: 일시적인 정보 획득 실패<br>Halt: 일시정지 상태 |
...
스토리지 복제를 설정합니다.
복제를 설정하면 선택한 리전에 원본 스토리지와 같은 크기의 대상 스토리지가 생성됩니다. 대상 스토리지는 읽기 전용 상태로 생성되며, 대상 스토리지의 상태를 변경하려면 복제 중지를 하거나 복제 설정을 해제해야 합니다.
