...
Encrypted volume stores symmetric keys used for encryption in a key store in the NHN Cloud Secure Key Manager service. Therefore, to create encrypted volume, you must [create a key store](https://docs.nhncloud.com/en/Security/Secure%20Key%20Manager/ko/getting-started/#_1) in the Secure Key Manager service in advance. [Check the ID of the key store](https://docs.nhncloud.com/en/Security/Secure%20Key%20Manager/ko/getting-started/#_2) and enter it in the encryption key store settings.
...
> [Caution]
> To change the size of a replicated target volume, you must stop replication.
>
> The size of the target volume must be equal to or larger than the source volume. It is recommended to set the sizes of the source and target volumes to be equal.
...
| &lt;mount point&gt; | Directory on which volume is mounted<br/>Example: /mnt |

<a id="connect_volume.cifs"></a>
### CIFS

In File Explorer, right-click **My PC** in the left navigation pane and select **Connect Network Drive**. In the Connect Network Drive window, select the drive you want to mount and enter the folder path. The folder path is formatted as `\\{volume IP}\{volume name}`, for example: \\\\192.168.0.100\\cifs
