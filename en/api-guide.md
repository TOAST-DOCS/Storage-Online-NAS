...
| $[ prefix ]$directionChangedAt | Body | String | Time the replication direction was changed |
| $[ prefix ]$dstProjectId | Body | String | Project ID of the target volume |
| $[ prefix ]$dstRegion | Body | String | Target volume region |
| $[ prefix ]$dstTenantId | Body | String | Target volume tenant ID |
...
| $[ prefix ]$snapshotPolicy.maxScheduledCount | Body | Integer | Maximum number of snapshots that can be stored |
...
| $[ prefix ]$snapshotPolicy.schedule.timeOffset | Body | String | Reference time zone for snapshot auto-creation |
...
| $[ prefix ]$snapshotPolicy.maxScheduledCount | Body | Integer | N | Maximum number of snapshots that can be stored<br>You can set a maximum of 30, and the first automatically created snapshot will be deleted when the maximum number of saves is reached. |
...
| $[ prefix ]$snapshotPolicy.schedule.timeOffset | Body | String | N | Reference time zone for snapshot auto-creation |
...
| $[ prefix ]$preserved | Body | Boolean | Whether the snapshot is marked as undeletable by the system |
