...
This document describes how to manage volumes and snapshots through the APIs provided by NHN Cloud NAS.
...
!!! tip "Note: Setting up encryption key storage"
    When an encrypted volume is created, the symmetric key used for encryption is stored in the NHN Cloud Secure Key Manager service's keystore. To create encrypted volume, you must first [create a keystore](https://docs.nhncloud.com/ko/Security/Secure%20Key%20Manager/ko/getting-started/#_1) in the Secure Key Manager service. [Check the keystore ID](https://docs.nhncloud.com/ko/Security/Secure%20Key%20Manager/ko/getting-started/#_2) and enter it in the encryption keystore settings.
...
| volume.mirrors.direction | Body | String | Replication direction<br>- `FORWARD`: Source volume → Replica volume <br>- `REVERSE`: Replica volume → Source volume |
...
| --- | --- | --- | --- | --- |
| X-Auth-Token | Header | String | Y | Token ID |
| volume\_id | URL | String | Y | Volume ID to delete |
...
| --- | --- | --- | --- | --- |
| X-Auth-Token | Header | String | Y | Token ID |
| volume\_id | URL | String | Y | Volume ID to query |
...
| --- | --- | --- | --- | --- |
| X-Auth-Token | Header | String | O | Token ID |
| volume\_id | URL | String | O | Volume ID |
...
| X-Auth-Token | Header | String | O | Token ID |
| volume\_id | URL | String | O | Volume ID |
| interface\_id | URL | String | O | Interface ID to delete |
...
| X-Auth-Token | Header | String | O | Token ID |
| volume\_id | URL | String | O | Volume ID |
| snapshot\_id | URL | String | O | Snapshot ID |
...
!!! danger "주의"
    The size of the target volume for replication must be the same as the source volume. If the sizes of the source and target volumes differ, the replication may fail.
...
| --- | --- | --- | --- | --- |
| X-Auth-Token | Header | String | Y | Token ID |
| volume\_id | URL | String | Y | Source volume ID |
