...
!!! tip "Note"
    For more information about how to specify explicit resource dependencies, see [Resource dependencies](https://developer.hashicorp.com/terraform/tutorials/configuration-language/dependencies) in the Terraform documentation.
...
!!! danger "Caution"
    Changing the values of replication settings resources may delete the existing resources and create new ones, but the existing target volume will not be deleted.
    If the remaining target volume and the new target volume have the same name, creation may fail. Please be careful.
