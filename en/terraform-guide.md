...
!!! tip "Note"
    For more information about how to specify explicit resource dependencies, see the [Resource dependencies](https://developer.hashicorp.com/terraform/tutorials/configuration-language/dependencies) documentation from Terraform.
...
!!! danger "Caution"
    Changing the values of a replication settings resource may delete the existing resource and create a new one, but the existing target volume will not be deleted.
    Be aware that creation may fail if the remaining target volume and the new target volume have the same name.
