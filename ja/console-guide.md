...
| 複製設定状態 | 原本と対象ストレージ間の複製設定状態を表示します。<br>Active:有効化状態<br>Retrieve failed:一時的な情報取得失敗<br>Halt:一時停止状態 |
...
ストレージ複製を設定します。
複製を設定すると、選択したリージョンにソースストレージと同じサイズのターゲットストレージが作成されます。ターゲットストレージは読み取り専用状態で作成され、ターゲットストレージの状態を変更するには、複製を停止するか、複製設定を解除する必要があります。
