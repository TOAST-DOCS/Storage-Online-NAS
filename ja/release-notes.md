...
* NAS リソースアクセス権限の細分化
    * Infrastructure NAS ADMIN ロールが追加され、NAS リソースの作成、変更、削除権限が提供されます。
...
* ボリューム状態表示の改善
    * ボリュームの状態が失敗またはエラーの場合、その原因を確認できるよう詳細な理由が併せて提供されます。
...
* CIFS認証情報 ID の制約条件変更
* ボリューム複製機能の変更
    * 複製方向変更後、即時に複製が有効化されます。
    * 複製を停止するか複製方向変更を行う場合、リクエスト時点までのデータが複製された後に処理が進行します。
...
### 新規リージョン追加 { #november-23-2022-added-new-region }

* 韓国（坪村）リージョンにて NAS サービスをリリース
