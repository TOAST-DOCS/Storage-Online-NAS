...
### 新機能追加 { #august-25-2026-added-features }

* NAS リソースアクセス権限の細分化
    * Infrastructure NAS ADMIN ロールが追加され、NAS リソースの作成、変更、削除権限を提供します。
...
* ボリュームステータス表示の改善
    * ボリュームのステータスが失敗またはエラーの場合、その原因を確認できるよう詳細な理由が合わせて提供されます。
...
* CIFS認証情報 ID の制約条件変更
* ボリューム複製機能の変更
    * 複製方向変更後、即座に複製が有効化されます。
    * 複製を停止または複製方向変更を行う場合、リクエスト時点までのデータが複製された後に処理が進みます。
